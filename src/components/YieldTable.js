        ...
        <Row>
          {pool.vaultUrl && (
            <Col>
              <b>
                {
                  <a href={pool.vaultUrl} target="_blank" rel="noreferrer">
                    Deposit on Beefy / View APY Chart
                  </a>
                }
              </b>
            </Col>
          ...
          {(pool.addLiquidityUrl || pool.buyTokenUrl) && (
            <Col>
              {pool.addLiquidityUrl && (
                <a href={pool.addLiquidityUrl} target="_blank" rel="noreferrer">
                  Add Liquidity
                </a>
              )}{" "}
              {pool.addLiquidityUrl && pool.buyTokenUrl && " | "}{" "}
              {pool.buyTokenUrl && (
                <a href={pool.buyTokenUrl} target="_blank" rel="noreferrer">
                  Buy Token
                </a>
              )}
            </Col>
...
const formatApyAndAmount = (apy, amount) => (
  <>
    ...
    {amount && formatFiat(amount) !== "" ? (
      <span className={"text-success apy-fiat-text"}>{formatFiat(amount)}</span>
    ) : (
      ""
    )}
  </>
...
const apyFormatter = (totalApy, { totalApyAmount }) =>
  formatApyAndAmount(totalApy, totalApyAmount);
const monthlyFormatter = (monthlyApy, { monthlyApyAmount }) =>
  formatApyAndAmount(monthlyApy, monthlyApyAmount);
const dailyFormatter = (dailyApy, { dailyApyAmount }) =>
  formatApyAndAmount(dailyApy, dailyApyAmount);

const withdrawalFeeFormatter = (withdrawalFee, { dailyApy }) => (
  <span
    className={
      numeral(withdrawalFee).value() === 0
        ? ""
        : numeral(withdrawalFee).value() > dailyApy
        ? "text-danger"
        : ""
    }
  >
    {withdrawalFee === "0%" ? "-" : withdrawalFee}
  </span>
);

const depositFeeFormatter = (depositFee, { dailyApy }) => (
  <span
    className={
      numeral(depositFee).value() === 0
        ? ""
        : numeral(depositFee).value() > dailyApy
        ? "text-danger"
        : ""
    }
  >
...
const safetyScoreFormatter = (safetyScore) => (
  <span
    className={
      numeral(safetyScore).value() === 9.991337
        ? "text-warning"
        : numeral(safetyScore).value() === 10
        ? "text-success"
        : numeral(safetyScore).value() <= 8
        ? "text-danger"
        : ""
    }
  >
    {safetyScore === 9.991337
      ? "?"
      : safetyScore === 10
      ? "✓"
      : numeral(safetyScore).format("0.0")}
  </span>
);
/*
...
const tvlFormatter = (tvl) => (
  <span
    className={
      numeral(tvl).value() >= 1000000
        ? "text-success"
        : numeral(tvl).value() < 100000
        ? "text-danger"
        : ""
    }
  >
